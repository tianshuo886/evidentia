...
Adds Phase-A provenance: every inventory binds source_sha256 and records
binding_method / binding_confidence / needs_visual_review (fail-closed review gate)."""
  ...
  for b in page.get_text('blocks'):
   for line in b[4].splitlines():
    ...
    if best:
     ...
     n+=1;fn=f'fig{n:02d}_p{pno+1}_{pix.width}x{pix.height}.png';pix.save(os.path.join(a.out,fn));saved.append(xref);item['file']=f'assets/figures/{fn}';item['extraction_method']='embedded';item['confidence']=0.9;item['binding_method']='embedded';item['binding_confidence']=0.9
    items.append(item)
 for item in items:
  if item['file'] is None and item['kind']=='figure':
   page=doc[item['page']-1];pix=page.get_pixmap(dpi=a.dpi,alpha=False);fn=f"page_p{item['page']}_{pix.width}x{pix.height}.png";pix.save(os.path.join(a.out,fn));item['file']=f'assets/figures/{fn}';item['extraction_method']='page_crop';item['confidence']=0.55;item['binding_method']='page_crop';item['binding_confidence']=0.55
 for item in items:
  item['needs_visual_review']=bool(item.get('confidence',0)<0.8 or item.get('binding_confidence',0)<0.8)
 inv={'schema_version':'1.0','source_sha256':sha256(a.pdf),'pdf':os.path.abspath(a.pdf),'pages':len(doc),'items':items,'embedded_saved':len(saved),'unmatched_assets':[],'review_required':[i['id'] for i in items if i.get('needs_visual_review')]}

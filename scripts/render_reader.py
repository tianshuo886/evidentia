...
def main():
 ...
 title=pm.get('paper',{}).get('title','Untitled paper'); claims=pm.get('claims',[]);figs=pm.get('figures',[]) or [x for x in inv.get('items',[]) if x.get('kind')=='figure'];tables=pm.get('tables',[]) or [x for x in inv.get('items',[]) if x.get('kind')=='table']
 ...
 body+=card('Evidence atlas',''.join(f'<article class="evidence" id="{esc(x.get("id"))}"><h3>{esc(x.get("paper_label",x.get("id")))}</h3><p>{esc(x.get("caption_original"))}</p><p><small>p.{esc(x.get("page"))} · {esc(x.get("role"))} · {esc(x.get("depth"))}</small></p>'+ (f'<img src="../{esc(x.get("file"))}" alt="{esc(x.get("paper_label"))}">' if x.get('file') else '')+'</article>' for x in figs+tables))
 ...
 ir={'schema_version':'1.0','paper_id':pm.get('paper_id',''),'dashboard':{'title':title,'claim_count':len(claims)},'paper_map':pm.get('natural_structure',[]),'claim_cards':[c.get('id') for c in claims],'figure_blocks':[x.get('id') for x in figs],'table_blocks':[x.get('id') for x in tables],'delta_projects':[d.get('project',{}).get('name','') for d in deltas]};(r/'reader/render_ir.json').write_text(json.dumps(ir,indent=2,ensure_ascii=False)+'\n')
 try:
